...
export default {
  ...
  methods: {
    ...
    async handleUpload(uploadData: object, nameOfCreatedFile: string) {
      try {
        const jsonString = JSON.stringify(uploadData);

        const blob = new Blob([jsonString], { type: 'application/json' });

        const file = new File([blob], `${nameOfCreatedFile}.json`, { type: 'application/json' });

        const formData = new FormData();
        formData.append(nameOfCreatedFile, file);
        ...
        console.log('__A_AS_D_ASD', response.data)
      ...
      try {
        ...
        this.message = `File uploaded successfully: ${key}`
      } catch (error) {
        this.message = `File upload failed: ${error?.message}`
      }
    }
  }
}
      ...
      <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div>
            ...
            <label :for="registerToLobby.key" class="block text-l font-small text-gray-700">{{ registerToLobby.label }}</label>
          ...
          <div>
            ...
            <label :for="maxConnections.key" class="block text-l font-small text-gray-700">{{ maxConnections.label }}</label>
            <InputText :id="maxConnections.key" class="w-full mt-2" v-model="maxConnections.value" />
          </div>
        </div>
      </div>
      <div>
        <button class="mt-2 bg-zinc-200 hover:bg-blue-dark font-bold py-2 px-4 rounded" @click="updateConfiguration">Update Configuration</button>
      </div>
      ...
      <div class="space-y-4 mt-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <label :for="adminPassword.key" class="block text-l font-small text-gray-700">{{ adminPassword.label }}</label>
          ...
          <div>
            ...
            <label :for="safetyRatingRequirement.key" class="block text-l font-small text-gray-700">{{ safetyRatingRequirement.label }}</label>
          ...
          <div>
            ...
            <label :for="spectatorPassword.key" class="block text-l font-small text-gray-700">{{ spectatorPassword.label }}</label>
          ...
          <div>
            ...
            <label :for="dumpLeaderboards.key" class="block text-l font-small text-gray-700">{{ dumpLeaderboards.label }}</label>
          ...
          <div>
            ...
            <label :for="dumpEntryList.key" class="block text-l font-small text-gray-700">{{ dumpEntryList.label }}</label>
          ...
          <div>
            ...
            <label :for="isRaceLocked.key" class="block text-l font-small text-gray-700">{{ isRaceLocked.label }}</label>
          ...
          <div>
            ...
            <label :for="shortFormationLap.key" class="block text-l font-small text-gray-700">{{ shortFormationLap.label }}</label>
          ...
          <div>
            ...
            <label :for="formationLapType.key" class="block text-l font-small text-gray-700">{{ formationLapType.label }}</label>
          ...
          <div>
            ...
            <label :for="doDriverSwapBroadcast.key" class="block text-l font-small text-gray-700">{{ doDriverSwapBroadcast.label }}</label>
          ...
          <div>
            ...
            <label :for="configVersion.key" class="block text-l font-small text-gray-700">{{ configVersion.label }}</label>
            <InputText :id="configVersion.key" class="w-full mt-2" v-model="configVersion.value" />
          </div>
        </div>
      </div>
      <div>
        <button class="mt-2 bg-zinc-200 hover:bg-blue-dark font-bold py-2 px-4 rounded" @click="updateSettings">Update Settings</button>
      </div>
      ...
      <div class="space-y-4 mt-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <label :for="preRaceWaitingTimeSeconds.key" class="block text-l font-small text-gray-700">{{ preRaceWaitingTimeSeconds.label }}</label>
            <InputText :id="preRaceWaitingTimeSeconds.key" class="w-full mt-2" v-model="preRaceWaitingTimeSeconds.value" />
          </div>
          <div>
            <p class="mb-2">Key: {{sessionOverTimeSeconds.key}} - Type: {{sessionOverTimeSeconds.type}}</p>
            <label :for="sessionOverTimeSeconds.key" class="block text-l font-small text-gray-700">{{ sessionOverTimeSeconds.label }}</label>
          ...
          <div>
            ...
            <label :for="weatherRandomness.key" class="block text-l font-small text-gray-700">{{ weatherRandomness.label }}</label>
          ...
          <div>
            ...
            <label :for="postQualySeconds.key" class="block text-l font-small text-gray-700">{{ postQualySeconds.label }}</label>
          ...
          <div>
            ...
            <label :for="postRaceSeconds.key" class="block text-l font-small text-gray-700">{{ postRaceSeconds.label }}</label>
          ...
          <div>
            ...
            <label :for="simracerWeatherConditions.key" class="block text-l font-small text-gray-700">{{ simracerWeatherConditions.label }}</label>
            <InputText :id="simracerWeatherConditions.key" class="w-full mt-2" v-model="simracerWeatherConditions.value" />
          </div>
          <div>
            <p class="mb-2">Key: {{isFixedConditionQualification.key}} - Type: {{isFixedConditionQualification.type}}</p>
            <label :for="isFixedConditionQualification.key" class="block text-l font-small text-gray-700">{{ isFixedConditionQualification.label }}</label>
            <InputText :id="isFixedConditionQualification.key" class="w-full mt-2" v-model="isFixedConditionQualification.value" />
          </div>
          <div>
          ...
          <div>
            ...
            <label :for="dayOfWeekend.key" class="block text-l font-small text-gray-700">{{ dayOfWeekend.label }}</label>
          ...
          <div>
            ...
            <label :for="timeMultiplier.key" class="block text-l font-small text-gray-700">{{ timeMultiplier.label }}</label>
          ...
          <div>
            ...
            <label :for="sessionDurationMinutes.key" class="block text-l font-small text-gray-700">{{ sessionDurationMinutes.label }}</label>
            <InputText :id="sessionDurationMinutes.key" class="w-full mt-2" v-model="sessionDurationMinutes.value" />
          </div>
        </div>
      </div>
      <div>
        <button class="mt-2 bg-zinc-200 hover:bg-blue-dark font-bold py-2 px-4 rounded" @click="test">Update Event</button>
      </div>
      ...
      <div class="space-y-4 mt-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div>
            ...
            <label :for="qualifyStandingType.key" class="block text-l font-small text-gray-700">{{ qualifyStandingType.label }}</label>
          ...
          <div>
            ...
            <label :for="superpoleMaxCar.key" class="block text-l font-small text-gray-700">{{ superpoleMaxCar.label }}</label>
          ...
          <div>
            ...
            <label :for="pitWindowLengthSec.key" class="block text-l font-small text-gray-700">{{ pitWindowLengthSec.label }}</label>
          ...
          <div>
            ...
            <label :for="driverStintTimeSec.key" class="block text-l font-small text-gray-700">{{ driverStintTimeSec.label }}</label>
          ...
          <div>
            ...
            <label :for="isRefuellingAllowedInRace.key" class="block text-l font-small text-gray-700">{{ isRefuellingAllowedInRace.label }}</label>
            <InputText :id="isRefuellingAllowedInRace.key" class="w-full mt-2" v-model="isRefuellingAllowedInRace.value" />
          </div>
          <div>
            <p class="mb-2">Key: {{isRefuellingTimeFixed.key}} - Type: {{isRefuellingTimeFixed.type}}</p>
            <label :for="isRefuellingTimeFixed.key" class="block text-l font-small text-gray-700">{{ isRefuellingTimeFixed.label }}</label>
          ...
          <div>
            ...
            <label :for="mandatoryPitstopCount.key" class="block text-l font-small text-gray-700">{{ mandatoryPitstopCount.label }}</label>
          ...
          <div>
            ...
            <label :for="maxTotalDrivingTime.key" class="block text-l font-small text-gray-700">{{ maxTotalDrivingTime.label }}</label>
          ...
          <div>
            ...
            <label :for="maxDriversCount.key" class="block text-l font-small text-gray-700">{{ maxDriversCount.label }}</label>
            <InputText :id="maxDriversCount.key" class="w-full mt-2" v-model="maxDriversCount.value" />
          </div>
          <div>
            <p class="mb-2">Key: {{isMandatoryPitstopRefuellingRequired.key}} - Type: {{isMandatoryPitstopRefuellingRequired.type}}</p>
            <label :for="isMandatoryPitstopRefuellingRequired.key" class="block text-l font-small text-gray-700">{{ isMandatoryPitstopRefuellingRequired.label }}</label>
            <InputText :id="isMandatoryPitstopRefuellingRequired.key" class="w-full mt-2" v-model="isMandatoryPitstopRefuellingRequired.value" />
          </div>
          <div>
            <p class="mb-2">Key: {{isMandatoryPitstopTyreChangeRequired.key}} - Type: {{isMandatoryPitstopTyreChangeRequired.type}}</p>
            <label :for="isMandatoryPitstopTyreChangeRequired.key" class="block text-l font-small text-gray-700">{{ isMandatoryPitstopTyreChangeRequired.label }}</label>
            <InputText :id="isMandatoryPitstopTyreChangeRequired.key" class="w-full mt-2" v-model="isMandatoryPitstopTyreChangeRequired.value" />
          </div>
          <div>
            <p class="mb-2">Key: {{isMandatoryPitstopSwapDriverRequired.key}} - Type: {{isMandatoryPitstopSwapDriverRequired.type}}</p>
            <label :for="isMandatoryPitstopSwapDriverRequired.key" class="block text-l font-small text-gray-700">{{ isMandatoryPitstopSwapDriverRequired.label }}</label>
            <InputText :id="isMandatoryPitstopSwapDriverRequired.key" class="w-full mt-2" v-model="isMandatoryPitstopSwapDriverRequired.value" />
          </div>
          <div>
            <p class="mb-2">Key: {{tyreSetCount.key}} - Type: {{tyreSetCount.type}}</p>
            <label :for="tyreSetCount.key" class="block text-l font-small text-gray-700">{{ tyreSetCount.label }}</label>
            <InputText :id="tyreSetCount.key" class="w-full mt-2" v-model="tyreSetCount.value" />
          </div>
        </div>
      </div>
      <div>
        <button class="mt-2 bg-zinc-200 hover:bg-blue-dark font-bold py-2 px-4 rounded" @click="test">Update Event Rules</button>
      </div>
    </div>
  </div>
</template>
